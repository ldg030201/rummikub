...
- **재접속**: 최초 join 시 서버가 비밀 `reconnectToken` 발급 → 클라가 sessionStorage에 저장 → 새로고침/재연결 시 토큰으로 좌석 복귀. 토큰이 없으면(탭 닫고 재입장) **이름+방코드 일치 시 끊긴 좌석에 한해 복귀** 폴백. 이 폴백은 인증이 아니라(방 코드·이름은 방 전원에게 보인다) 좌석 탈취 경로이므로 세 겹으로 좁혔다 — ①끊긴 지 `NAME_REATTACH_MS`(5분) 이내만 ②그렇게 들어온 좌석은 `authed:false`라 `settings` 변경 불가(패 공개를 켜면 방 전원 손패가 노출되므로 파급이 크다) ③복귀 사실을 시스템 채팅으로 방에 알린다. 정상 사용자가 폴백에 의존하지 않게, 클라는 `reconnectToken`을 sessionStorage뿐 아니라 localStorage `rk_tok_<방>_<이름>`에도 사본으로 둔다(방+이름 키라 같은 브라우저 2인 플레이도 안 꼬인다). 미제출 draft와 턴은 **유예 타이머(45s)** 로 보존돼 새로고침해도 안 뺏긴다.
