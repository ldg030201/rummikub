...
- **패 공개(디버그)**: 방 설정 `revealHands`(기본 꺼짐, 대기실에서 방장만)를 켜면 서버가 `state.hands`(`{playerId: Tile[]}`)를 방 전원에게 내려준다. 클라는 **⌘⌃⌥P**(`metaKey+ctrlKey+altKey+code==='KeyP'` — Alt가 `e.key`를 바꾸므로 `e.code`로 판정)로 상대 좌석에 실제 패를 펼친다. 서버 `racks`는 커밋 전까지 안 줄어들어서, 지금 보드에 올려둔 타일은 손패에서 흐리게(`.placing`) 표시한다. **기본이 꺼짐인 이유**: 켜는 순간 방의 누구나(devtools로도) 남의 패를 볼 수 있으므로, 설정을 브로드캐스트해 몰래 켤 수 없게 하고 대기실에 경고를 띄운다. 단축키만으로는 아무것도 볼 수 없다.
- **터치 드래그**: HTML5 DnD(`dragstart`/`drop`)는 모바일 브라우저에서 발생하지 않아, `Game.jsx`가 포인터 이벤트(`pointerdown/move/up`, `pointerType !== 'mouse'`)로 드래그를 흉내낸다. 8px 넘게 움직여야 드래그로 판정(탭과 구분)하고, 손가락을 따라다니는 클론(`.touch-ghost`)을 body에 띄운 뒤, 놓는 순간 `elementFromPoint`로 대상을 찾아 **기존 드롭 핸들러**(`dropOnSlot`/`dropOnGrid`/`dropIntoMeld`/`dropIntoNewMeld`/`dropOnFelt`)에 합성 이벤트를 넘긴다 — 드롭 로직은 이중화하지 않는다. 타일에 `touch-action: none`이라 타일 위 스와이프는 스크롤이 아니라 드래그다. 마우스는 그대로 네이티브 DnD. (터치엔 Shift가 없어 블럭 드래그는 마우스 전용 — 정렬 버튼으로 대체)
