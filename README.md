...
- **방 설정** — 턴 제한시간(30초~3분·무제한), 최대 인원, 타일 세트 수, 패 공개(디버그) (대기실에서 방장이 변경)
- **패 공개 (디버그, 기본 꺼짐)** — 방장이 켜면 방 전원이 게임 화면의 「패 보기」 버튼으로 서로의 손패와 그 변화를 볼 수 있어. 켜고 끌 때마다 시스템 채팅이 남고 게임 화면에 경고가 상시 표시돼 몰래 켤 수 없어 (룰 디버깅·훈수용)
